...
def hits_and_ranks(examples, scores, all_answers, verbose=False):
    ...
    for i, example in enumerate(examples):
        ...
        target_score = float(scores.asnumpy()[i, e2])
    ...
    # sort and rank
    ms_topk=mindspore.ops.TopK()
    #op_k_scores, top_k_targets = torch.topk(scores, min(scores.size(1), args.beam_size))
    top_k_scores, top_k_targets = ms_topk(scores, min(scores.shape[1], args.beam_size))
    # topk => return values and its index
    #top_k_targets = top_k_targets.cpu().numpy()
    top_k_targets = top_k_targets.asnumpy()
...
def hits_at_k(examples, scores, all_answers, verbose=False):
    ...
    ms_topk=mindspore.ops.TopK()
    top_k_scores, top_k_targets = ms_topk(scores, min(scores.size(1), args.beam_size))
    top_k_targets = top_k_targets.cpu().numpy()
