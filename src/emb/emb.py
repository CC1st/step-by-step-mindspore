...
class EmbeddingBasedMethod(LFramework):
    def __init__(self, args, kg, mdl, secondary_kg=None, tertiary_kg=None):
        ...
        self.loss_fun = mindspore.nn.BCELoss()

        self.theta = args.theta
        self.secondary_kg = secondary_kg
        self.tertiary_kg = tertiary_kg
        #mindspore->torch.cat()
        self.ms_concat=mindspore.ops.Concat()
        self.ms_cast=mindspore.ops.Cast()
        #mindspore->torch.sum()
        self.ms_reductSum=mindspore.ops.ReduceSum()
    ...
    def forward_fact(self, examples):
        ...
        return self.ms_concat(pred_scores)

    #mindspore
    def construct(self,examples):
        out=self.forward_fact(examples)
        return out
    ...
    def loss(self, mini_batch):
        ...
        loss_dict['print_loss'] = mindspore.ops.Cast()(loss, mindspore.float32)
        return loss_dict
    ...
    def get_subject_mask(self, e1_space, e2, q):
        ...
        for i in range(len(e1_space)):
            ...
            if not _e2 in answer_vectors or not _q in answer_vectors[_e2]:
                ...
                answer_vector = var_cuda(mindspore.Tensor([[kg.num_entities]], mindspore.int32))
            else:
                answer_vector = answer_vectors[_e2][_q]
            #subject_mask = torch.sum(e1_space[i].unsqueeze(0) == answer_vector, dim=0)
            subject_mask = mindspore.Tensor.sum(mindspore.ops.ExpandDims()(e1_space[i], 0) == answer_vector, axis=0)
            subject_masks.append(subject_mask)
        #subject_mask = torch.cat(subject_masks).view(len(e1_space), -1)
        subject_mask = mindspore.ops.Concat()(subject_masks).view((len(e1_space), -1))
        return subject_mask
    ...
    def get_object_mask(self, e2_space, e1, q):
        ...
        for i in range(len(e2_space)):
            ...
            object_mask = mindspore.Tensor.sum(mindspore.ops.ExpandDims()(e2_space[i], 0) == answer_vector, axis=0)
            object_masks.append(object_mask)
        #object_mask = torch.cat(object_masks).view(len(e2_space), -1)
        object_mask = mindspore.ops.Concat()(object_masks).view((len(e2_space), -1))
        return object_mask
    ...
    def export_reward_shaping_parameters(self):
        ...
        mindspore.save_checkpoint(self.mdl.state_dict(), fn_state_dict_path)
        print('Fact network parameters export to {}'.format(fn_state_dict_path))
        #torch.save(self.kg.state_dict(), fn_kg_state_dict_path)
        mindspore.save_checkpoint(self.kg.state_dict(), fn_kg_state_dict_path)
    ...
    def export_fuzzy_facts(self):
        ...
        for i in range(0, len(e1_ids), self.batch_size):
            ...
            e1 = var_cuda(mindspore.Tensor(e1_ids_b, mindspore.int32))
            r = var_cuda(mindspore.Tensor(r_ids_b, mindspore.int32))
            pred_scores = mdl.forward(e1, r, kg)
        ...
        e1 = int_var_cuda(mindspore.numpy.arange(kg.num_entities))
        for i in range(len(e2_ids)):
            #r = int_fill_var_cuda(e1.size(), r_ids[i])
            #e2 = int_fill_var_cuda(e1.size(), e2_ids[i])
            r = int_fill_var_cuda(e1.shape, r_ids[i])
            e2 = int_fill_var_cuda(e1.shape, e2_ids[i])
