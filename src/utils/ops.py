...
EPSILON = float(np.finfo(np.float).eps)
HUGE_INT = 1e31
...
def batch_lookup(M, idx, vector_output=True):
    ...
    batch_size, w = M.shape
    batch_size2, sample_size = idx.shape
    assert(batch_size == batch_size2)

    if sample_size == 1 and vector_output:
        #samples = torch.gather(M, 1, idx).view(-1)
        samples = mindspore.ops.GatherD()(M, 1, idx).view(-1) #graph: %147 :323
    else:
        #samples = torch.gather(M, 1, idx)
        samples = mindspore.ops.GatherD()(M, 1, idx)
    return samples
...
def entropy(p):
    ...
    return mindspore.ops.ReduceSum()(-p * safe_log(p), -1)


def weighted_softmax(v, w, dim=-1):
    #exp_v = torch.exp(v)
    exp_v = mindspore.ops.Exp()(v)
    weighted_exp_v = w * exp_v
    #return weighted_exp_v / torch.sum(weighted_exp_v, dim, keepdim=True)
    return weighted_exp_v / mindspore.ops.ReduceSum(keep_dims=True)(weighted_exp_v, dim)
...
def pad_and_cat(a, padding_value, padding_dim=1):
    ...
    for x in a:
        ...
        if x.shape[padding_dim] < max_dim_size:
            res_len = max_dim_size - x.shape[padding_dim]
            pad = mindspore.nn.Pad((0, res_len), mode="CONSTANT")
            #mindspore.nn.Pad只能填充常数0，故填充前减去填充值，填充后加上
            pad_out=pad(x-padding_value)+padding_value
            padded_a.append(pad_out)
        else:
            padded_a.append(x)
    #return torch.cat(padded_a, dim=0)
    return mindspore.ops.Concat(0)(padded_a)
...
def safe_log(x):
    ...
    return mindspore.ops.Log()(x + EPSILON) #param must be float
...
def tile_along_beam(v, beam_size, dim=0):
    ...
    #v = v.unsqueeze(dim + 1)
    v = mindspore.ops.ExpandDims()(v, dim + 1)
    #v = torch.cat([v] * beam_size, dim=dim+1)
    v = mindspore.ops.Concat(axis=(dim+1))([v] * beam_size)
    new_size = []
...
def unique_max(unique_x, x, values, unique_interval=100, marker_2D=None):
    ...
    for i in range(0, len(unique_x), unique_interval):
        ...
        marker_2D = mindspore.ops.Cast()(mindspore.ops.ExpandDims()(unique_x_b, 1) == (mindspore.ops.ExpandDims()(x, 0))
                                         , mindspore.float32) # size([33, 33])
        values_2D = marker_2D * mindspore.ops.ExpandDims()(values, 0) - (1 - marker_2D) * HUGE_INT
        #unique_values_b, unique_idx_b = values_2D.max(dim=1)
        unique_idx_b, unique_values_b = mindspore.ops.ArgMaxWithValue(1)(values_2D)
    ...
    unique_values = mindspore.ops.Concat()(unique_values)
    unique_idx = mindspore.ops.Concat()(unique_indices)
    return unique_values, unique_idx

if __name__ == '__main__':
    ms_standardNormal=mindspore.ops.StandardNormal()
    #a = torch.randn(2)
    a = ms_standardNormal((2))
    print(a)
    print(tile_along_beam(a, 4))
    print('--------------------------')
    #b = torch.randn(2, 3)
    b = ms_standardNormal((2, 3))
    print(b)
