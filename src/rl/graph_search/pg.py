...
class PolicyGradient(LFramework):
    ...
    def reward_fun(self, e1, r, e2, pred_e2):
        ...
        return self.ms_cast((pred_e2 == e2), mindspore.float32)
    ...
    def loss_hrl(self, mini_batch):
        ...
        def stablize_reward(r):
            ...
            if self.baseline == 'avg_reward':
                ...
                stabled_r_2D = r_2D - r_2D.mean(axis=1, keep_dims=True)
                ...
                stabled_r_2D = (r_2D - r_2D.mean(axis=1, keep_dims=True)) / (r_2D.std(axis=1, keepdims=True) + ops.EPSILON)
            else:
        ...
        for i in range(self.num_rollout_steps):
            ...
            pt_loss_high += -cum_discounted_rewards_high[i] * mindspore.ops.Exp()(log_option_prob)
            ...
            pt_loss_low += -cum_discounted_rewards_low[i] * mindspore.ops.Exp()(log_action_prob)

        # high level entropy regularization
        #entropy_high = torch.cat([x.unsqueeze(1) for x in action_entropy], dim=1).mean(dim=1)
        entropy_high = (mindspore.ops.Concat(1)([mindspore.ops.ExpandDims()(x, 1) for x in action_entropy])).mean(axis=1)
        ...
        entropy_low = (mindspore.ops.Concat(1)([mindspore.ops.ExpandDims()(x, 1) for x in action_entropy])).mean(axis=1)
        ...
        loss_dict = {'model_loss_high': pg_loss_high,
                     'print_loss_high': pt_loss_high.asnumpy(),
                     #'print_loss_high': mindspore.ops.Cast()(pt_loss_high, mindspore.float32),
                     'reward_high': reward_high,
                     'entropy_high': entropy_high.mean().asnumpy(),
                     #'entropy_high': mindspore.ops.Cast()(entropy_high.mean(), mindspore.float32),
                     'model_loss_low': pg_loss_low,
                     'print_loss_low': pt_loss_low.asnumpy(),
                     #'print_loss_low': mindspore.ops.Cast()(pt_loss_low,mindspore.float32),
                     'reward_low': reward_low,
                     'entropy_low': entropy_low.mean().asnumpy()}
                     #'entropy_low': mindspore.ops.Cast()(entropy_low.mean(), mindspore.float32)}

        if self.run_analysis:
            #fn = torch.zeros(reward_low.size())
            #fn = mindspore.ops.Zeros()(reward_low.shape, mindspore.float32)
            fn = np.zeros(reward_low.shape, dtype=np.float)
            for i in range(len(reward_low)):
                if not reward_low[i]:
                    all_object_x = self.kg.all_objects[int(e1[i])]
                    all_object_x = collections.defaultdict(set)
                    #if int(pred_e2[i]) in self.kg.all_objects[int(e1[i])][int(r[i])]:
                    if int(pred_e2[i]) in all_object_x[int(r[i])]:
                        fn[i] = 1
            fn = mindspore.Tensor(fn, mindspore.float32)
            loss_dict['fn'] = fn
    ...
    def sample_relation(self, db_outcomes, inv_offset=None):
        ...
        def apply_relation_dropout_mask(r_dist, r_mask):
            if self.relation_dropout_rate > 0:
                ...
                r_dist=mindspore.ops.stop_gradient(r_dist)
                rand = mindspore.ops.UniformReal()(r_dist.shape)
                #relation_keep_mask = var_cuda(rand > self.relation_dropout_rate).float()
                relation_keep_mask = mindspore.ops.Cast()(mindspore.Tensor(rand > self.relation_dropout_rate), mindspore.float32)
        ...
        def sample(r_space, r_mask, r_dist):
            ...
            sample_relation_dist=mindspore.ops.stop_gradient(sample_relation_dist)
            next_r_idx = mindspore.ops.Multinomial()(sample_relation_dist, 1)
            #next_r_idx = mindspore.Tensor(next_r_idx, mindspore.int64)
    ...
    def sample_tailentity(self, db_outcomes, next_r, r_prob, tailentity_entropy, inv_offset=None):
        ...
        def apply_tailentity_dropout_mask(te_dist, te_mask):
            if self.tailentity_dropout_rate > 0:
                ...
                te_dist=mindspore.ops.stop_gradient(te_dist)
                rand = mindspore.ops.UniformReal()(te_dist.shape)
                tailentity_keep_mask = mindspore.ops.Cast()(mindspore.Tensor(rand > self.tailentity_dropout_rate), mindspore.float32)
        ...
        def sample(te_space, te_mask, te_dist, next_r, r_prob, tailentity_entropy):
            ...
            sample_tailentity_dist = mindspore.ops.stop_gradient(sample_tailentity_dist)
            idx = mindspore.ops.Multinomial()(sample_tailentity_dist, 1)
    ...
    #def rollout_hrl(self, mini_batch,visualize_action_probs=False):
        """
        ...
        #e_s, e_t, q = self.format_batch(mini_batch, num_tiles=self.num_rollouts)
        # Initialization
        #r_s = int_fill_var_cuda(e_s.size(), kg.dummy_start_r)  # dummy_start_r => 1
        #r_s = mindspore.Parameter(mindspore.ops.Ones()(e_s.shape, mindspore.int32), requires_grad=False)

        r_s = mindspore.ops.Ones()(e_s.shape, mindspore.int32)

        #seen_nodes = int_fill_var_cuda(e_s.size(), kg.dummy_e).unsqueeze(1)  # dummy_e => 0
        zeros_seen_nodes = mindspore.ops.Zeros()(e_s.shape, mindspore.int32)
        seen_nodes = mindspore.ops.ExpandDims()(zeros_seen_nodes, 1)
        #seen_nodes = mindspore.Parameter(seen_nodes, requires_grad=False)
        #seen_nodes = mindspore.ops.ExpandDims()(int_fill_var_cuda(e_s.shape, kg.dummy_e), 1)  # dummy_e => 0
        ...
        for t in range(num_steps):  # num_steps: 3
            ...
            #next_te=mindspore.ops.Cast()(next_te, mindspore.int64)
            ...
            seen_nodes = mindspore.ops.Concat(1)((seen_nodes, mindspore.ops.ExpandDims()(e, 1)))
    ...
    def predict(self, mini_batch, verbose=False):
        ...
        #with torch.no_grad():
        pred_scores = zeros_var_cuda((len(e1), kg.num_entities))  # size([self.mini_batch_size, 137]) e.g., size([32, 137])
        for i in range(len(e1)):
            pred_e2_scores[i] = mindspore.ops.stop_gradient(pred_e2_scores[i])
            pred_scores[i][pred_e2s[i]] = mindspore.ops.Exp()(pred_e2_scores[i])
        return pred_scores
    ...
    def record_path_trace(self, path_trace):
        ...
        path_trace_mat = mindspore.ops.Concat()(flattened_path_trace).reshape((-1, path_length))
        path_trace_mat = mindspore.ops.stop_gradient(path_trace_mat).asnumpy()
