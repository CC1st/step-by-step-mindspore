...
from src.utils.ms_load_checkpoint import parameters_to_vector
...
class GraphSearchPolicy(mindspore.nn.Cell):
    def __init__(self, args, fn=None, fn_kg=None):
        ...
        # mindspore:将要连接的数据向较高的精度的参数精度转变
        self.ms_cast = mindspore.ops.Cast()
        # torch.nn.functional.relu
        self.ms_relu = mindspore.ops.ReLU()
        # torch.squeeze
        self.ms_squeeze = mindspore.ops.Squeeze()
        # torch.unsqueeze
        self.ms_unsqueeze = mindspore.ops.ExpandDims()
        # troch.max
        self.ms_max = mindspore.ops.ArgMaxWithValue()
        # troch.nn.functional.softmax()
        self.ms_softmax = mindspore.ops.Softmax()
        # troch.stack
        self.ms_stack = mindspore.ops.Stack()
        # torch.topk
        self.ms_topk = mindspore.ops.TopK(True)
        # torch.ones
        self.ms_ones = mindspore.ops.Ones()
        # torch.autograd._grad
        self.ms_autograd = mindspore.ops.GradOperation()
        # torch.zero
        self.ms_zeros = mindspore.ops.Zeros()
    ...
    def transit_high(self, e, obs, kg, use_relation_space_bucketing=True,
        ...
        def policy_nn_fun_high(X, relation_space, use_max_logit = False):
            ...
            if use_max_logit:
                ...
                logit = mindspore.ops.Squeeze(2)(R @ self.ms_unsqueeze(X, 2))  # => size([2560, 200, 1])
                _, max_logit = mindspore.ops.ArgMaxWithValue(axis=1,keep_dims=True)(logit)
                #无法找到detach对应的API
                max_logit.detach_()

                #option_dist = F.softmax(logit - max_logit - (1 - r_mask) * ops.HUGE_INT, dim=-1)
                option_dist = mindspore.ops.Softmax(axis=-1)(logit - max_logit - (1 - r_mask) * ops.HUGE_INT)

            else:
                #option_dist = F.softmax(
                #    torch.squeeze(R @ torch.unsqueeze(X, 2), 2) - (1 - r_mask) * ops.HUGE_INT, dim=-1) #@是矩阵叉乘
                #R_X=mindspore.ops.Squeeze(axis=2)(mindspore.ops.BatchMatMul()(R, mindspore.ops.ExpandDims()(X, 2)))
                #one_matirx=mindspore.ops.OnesLike()(r_mask)
                option_dist = mindspore.ops.Softmax(axis=-1)(
                    mindspore.ops.Squeeze(axis=2)(mindspore.ops.BatchMatMul()(R, mindspore.ops.ExpandDims()(X, 2)))
                    - (1 - r_mask) * ops.HUGE_INT)
            return option_dist, ops.entropy(option_dist) #graph: %165: 359
    ...
    def transit_low(self, e, next_r, obs, kg, r_space,
        ...
        if H.shape[0] < Q.shape[0]:
            ...
            H = ops.tile_along_beam(H, Q.shape[0] // H.shape[0])
        #elif H.size(0) > Q.size(0):
        elif H.shape[0] > Q.shape[0]:
            #k = H.size(0) // Q.size(0)
            #H = torch.stack([H[k * i] for i in range(Q.size(0))])
            k = H.shape[0] // Q.shape[0]
            H = mindspore.ops.Stack()([H[k * i] for i in range(Q.shape[0])])
        E = kg.get_entity_embeddings(e)
        ...
        if self.relation_only:
            ...
            if R.shape[0] > Q.shape[0]:
                ...
                H = ops.tile_along_beam(H, beam_size=R.shape[0] // H.shape[0], dim=0)
                Q = ops.tile_along_beam(Q, beam_size=R.shape[0] // Q.shape[0], dim=0)
            #X = torch.cat([E, Q, R, H], dim=-1)
            X = mindspore.ops.Concat(axis=-1)((E, Q, R, H)) #graph:%121: 271
        ...
        def policy_nn_fun_low(X, tailentity_space):
            ...
            action_dist = mindspore.ops.Softmax(axis=-1)(
                mindspore.ops.Squeeze(axis=-1)(mindspore.ops.BatchMatMul()(E, mindspore.ops.ExpandDims()(X, -1)))
                - (1 - te_mask) * ops.HUGE_INT)
        ...
        #mindspore.ops.Topk:dim参数指定最后一维，输出是：最大值序列 and 对应的index
        _, tail_selected = mindspore.ops.TopK()(tail_anticipated, self.additional_tailentity_size)
        mask_selected = mindspore.ops.Ones()(tail_selected.shape, mindspore.float32)
    ...
    def update_params(self, loss, step_size=0.5, first_order=False):
        ...
        return parameters_to_vector(self.get_parameters()) - parameters_to_vector(grads) * step_size
    ...
    def initialize_path(self, init_action, kg):
        ...
        init_action_embedding = mindspore.ops.ExpandDims()(init_action_embedding, 1)
        init_h = mindspore.ops.Zeros()((self.history_num_layers, len(init_action_embedding), self.history_dim), mindspore.float32)
        init_c = mindspore.ops.Zeros()((self.history_num_layers, len(init_action_embedding), self.history_dim), mindspore.float32)
    ...
    def update_path(self, action, kg, option_offset=None, action_offset=None):
        ...
        if last_hidden[0].shape[1] != action_embedding.shape[0]:
            k_tmp = action_embedding.shape[0] // last_hidden[0].shape[1]
            new_hidden_list = []
        ...
        self.path.append(self.path_encoder(mindspore.ops.ExpandDims()(action_embedding,1), last_hidden)[1])
    ...
    def get_tailentity_space(self, e, r, obs, kg, r_space):
        ...
        te_space = mindspore.ops.Zeros()((e.shape[0], kg.tailentity_space[0].shape[-1]))
        te_mask = mindspore.ops.Zeros()((e.shape[0], kg.tailentity_space[0].shape[-1]))
        ...
        te_space = ops.pad_and_cat(te_space, padding_value=0, padding_dim=0).view((e.shape[0], -1))
        te_mask = ops.pad_and_cat(te_mask, padding_value=0, padding_dim=0).view((e.shape[0], -1))
    ...
    def get_action_embedding(self, action, kg):
        ...
        if self.relation_only:
            ...
            action_embedding = mindspore.ops.Concat(axis=-1)((relation_embedding, entity_embedding))
    ...
    def define_modules(self):
        ...
        if self.rl_module == 'original':
            ...
            self.W1 = mindspore.nn.Dense(input_dim, self.action_dim)
            self.W2 = mindspore.nn.Dense(self.action_dim, self.action_dim)
            self.W1Dropout = mindspore.nn.Dropout(keep_prob=1-self.ff_dropout_rate)
            self.W2Dropout = mindspore.nn.Dropout(keep_prob=1-self.ff_dropout_rate)
        elif self.rl_module == 'hrl':
            ...
            self.W1 = mindspore.nn.Dense(input_dim_high, self.relation_dim)
            self.W2 = mindspore.nn.Dense(self.relation_dim, self.relation_dim)
            self.W3 = mindspore.nn.Dense(input_dim_low, self.entity_dim)
            self.W4 = mindspore.nn.Dense(self.entity_dim, self.entity_dim)
            ...
            self.W1Dropout = mindspore.nn.Dropout(keep_prob=1-self.ff_dropout_rate)
            self.W2Dropout = mindspore.nn.Dropout(keep_prob=1-self.ff_dropout_rate)
            self.W3Dropout = mindspore.nn.Dropout(keep_prob=1-self.ff_dropout_rate)
            self.W4Dropout = mindspore.nn.Dropout(keep_prob=1-self.ff_dropout_rate)

            #self.leakyrelu = nn.LeakyReLU(self.leakyrelu_slope)
            self.leakyrelu = mindspore.nn.LeakyReLU(self.leakyrelu_slope)
        ...
        if self.relation_only_in_path:
            ...
            #                            batch_first=True)
            self.path_encoder = mindspore.nn.LSTM(input_size=self.relation_dim,
                                                  hidden_size=self.history_dim,
                                                  num_layers=self.history_num_layers,
                                                  batch_first=True)
            ...
            #                            num_layers=self.history_num_layers,
            #                            batch_first=True)
            self.path_encoder = mindspore.nn.LSTM(input_size=self.action_dim,
                                                  hidden_size=self.history_dim,
                                                  num_layers=self.history_num_layers,
                                                  batch_first=True)
    ...
    def initialize_modules(self):
        if self.xavier_initialization:
            ...
            self.W1.weight = initializer(XavierUniform(), self.W1.weight.shape)
            self.W2.weight = initializer(XavierUniform(), self.W2.weight.shape)
            self.W3.weight = initializer(XavierUniform(), self.W3.weight.shape)
            self.W4.weight = initializer(XavierUniform(), self.W4.weight.shape)
            #for name, param in self.path_encoder.named_parameters():
                #if 'bias' in name:
                #    nn.init.constant_(param, 0.0)
                #elif 'weight' in name:
                #    nn.init.xavier_normal_(param)
            for param in self.path_encoder.get_parameters():
                #if 'bias' in param.name:
                if 'b_' in param.name:
                    #nn.init.constant_(param, 0.0)
                    param_array= np.asarray(param)
                    initializer(0.0, param_array, mindspore.float32)
                    param=mindspore.Tensor.from_numpy(param_array)

                elif 'weight' in param.name:
                    #nn.init.xavier_normal_(param)
                    param_array = initializer(XavierUniform(), param.shape, mindspore.float32)
                    param.set_data(param_array)
