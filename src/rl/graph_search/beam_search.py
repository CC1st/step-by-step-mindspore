...
def beam_search(pn, e_s, q, e_t, kg, num_steps, beam_size, beam_size_high, beam_size_low, rl_module='original',
    ...
    r_s = mindspore.ops.Ones()(e_s.shape, mindspore.int32)    #kg.dummy_start_r = 1
    #seen_nodes = int_fill_var_cuda(e_s.size(), kg.dummy_e).unsqueeze(1)
    seen_nodes = mindspore.ops.ExpandDims()(mindspore.ops.Zeros()(e_s.shape, mindspore.int32), 1)   #kg.dummy_e = 0
    init_action = (r_s, e_s)
    # path encoder
    ...
    log_option_prob = mindspore.ops.Zeros()(batch_size, mindspore.float32)
    log_tailentity_prob = mindspore.ops.Zeros()(batch_size * beam_size_high, mindspore.float32)
    log_action_prob = mindspore.ops.Zeros()((batch_size, 1), mindspore.float32)
    ...
    for t in range(num_steps):
        ...
        if e.shape[0] != q_init.shape[0]:
            k_temp = e.shape[0] // q_init.shape[0]
            # => [batch_size * k]
        ...
        assert (e.shape[0] == q.shape[0] == last_r.shape[0])
        ...
        if t == num_steps:
            ...
            for i in range(log_option_dist.shape[0]):
                ...
                #                                                              k_prime_high)
                top_unique_log_option_dist, top_unique_high_idx2 = mindspore.ops.TopK()(unique_log_option_dist,
                                                                             k=k_prime_high)
                ...
                next_r_list.append(mindspore.ops.ExpandDims()(top_r, 0))
                next_r_idxs.append(mindspore.ops.ExpandDims()(top_r_idx,0))
                log_option_prob_list.append(mindspore.ops.ExpandDims()(top_unique_log_option_dist,0))
                top_unique_batch_offset_high = i * 1
                top_unique_option_offset = top_unique_batch_offset_high + top_unique_beam_offset_high
                option_offset_list.append(mindspore.ops.ExpandDims()(top_unique_option_offset,0))
            ...
            #                                          k_high)
            log_option_prob, next_r_idx = mindspore.ops.TopK()(log_option_dist, k_high)
            ...
            #    1)  # [batch_size, 1]
            option_batch_offset = mindspore.ops.ExpandDims()(int_var_cuda(mindspore.numpy.arange(batch_size) * last_k_high),1) # [batch_size, 1]
            ...
            if option_batch_offset.shape[0] < option_beam_offset.shape[0]:
                option_batch_offset = ops.tile_along_beam(option_batch_offset, beam_size=option_beam_offset.shape[
                    0]// option_batch_offset.shape[0])
            option_offset = (option_batch_offset + option_beam_offset).view(-1)

        #if next_r.size(0) != e.size(0):
        #    k = next_r.size(0) // e.size(0)
        if next_r.shape[0] != e.shape[0]:
            k = next_r.shape[0] // e.shape[0]
        ...
        if t == 0:
            ...
            log_tailentity_prob = zeros_var_cuda(te_dist.shape[0])
        ...
        if log_tailentity_prob.shape[0] < te_dist.shape[0]:
            log_tailentity_prob = ops.tile_along_beam(log_tailentity_prob,
                                                      beam_size=te_dist.shape[0] // log_tailentity_prob.shape[0])
        ...
        if t == num_steps:
            ...
            action_space_size = te_space.shape[1]  # 116

            #beam_action_space_size = log_tailentity_dist.size()[1]  # 116?
            beam_action_space_size = log_tailentity_dist.shape[1]  # 116?
            ...
            for i in range(log_tailentity_dist.shape[0]):
                ...
                unique_te_space_b = var_cuda(mindspore.ops.Unique()(te_space_b.data.cpu()))
                ...
                #                                                                 k_prime)
                top_unique_log_tailentity_dist, top_unique_low_idx2 = mindspore.ops.TopK()(unique_log_tailentity_dist,
                                                                                k = k_prime)
                ...
                next_te_list.append(mindspore.ops.ExpandDims()(top_te, 0))
                next_te_idxs.append(mindspore.ops.ExpandDims()(top_te_idx, 0))
                log_tailentity_prob_list.append(mindspore.ops.ExpandDims()(top_unique_log_tailentity_dist, 0))
                ...
                action_offset_list.append(mindspore.ops.ExpandDims()(top_unique_action_offset, 0))
            ...
            action_space_size = te_space.shape[1]
            beam_action_space_size = log_tailentity_dist.shape[1]
            assert (beam_action_space_size % action_space_size == 0)
            ...
            #                                              k_low)
            log_tailentity_prob, next_te_idx = mindspore.ops.TopK()(log_tailentity_dist, k_low)
            ...
            action_batch_offset = int_var_cuda(mindspore.numpy.arange(batch_size) * last_k_high)
            if action_batch_offset.shape[0] < action_beam_offset.shape[0]:
                action_batch_offset = ops.tile_along_beam(action_batch_offset, beam_size=action_beam_offset.shape[
                    0] // action_batch_offset.shape[0])
            action_batch_offset = mindspore.ops.ExpandDims()(action_batch_offset.view(-1), 1)
        ...
        if log_option_prob.shape[0] < log_tailentity_prob.shape[0]:
            log_option_prob = ops.tile_along_beam(log_option_prob,
                                                  beam_size=log_tailentity_prob.shape[0] // log_option_prob.shape[0])
        elif log_option_prob.shape[0] > log_tailentity_prob.shape[0]:
            log_tailentity_prob = ops.tile_along_beam(log_tailentity_prob,
                                                      beam_size=log_option_prob.shape[0] // log_tailentity_prob.shape[0])
        ...
        if next_te.shape[0] > next_r.shape[0]:
            next_r = ops.tile_along_beam(next_r, beam_size=next_te.shape[0] // next_r.shape[0])
        ...
        if option_offset.shape[0] < action_offset.shape[0]:
            option_offset = ops.tile_along_beam(option_offset,
                                                beam_size=action_offset.shape[0] // option_offset.shape[0])
        option_offset = option_offset.view((batch_size, -1))
        action_offset = action_offset.view((batch_size, -1))
        ...
        if t == num_steps - 1:
            for i in range(batch_size):
                ...
                next_r_b_value, _ = mindspore.ops.Unique()(next_te_b)
                unique_te_b = var_cuda(next_r_b_value)
                ...
                top_unique_log_action_dist, top_unique_idx2 = mindspore.ops.TopK()(unique_log_action_dist, k_prime)
        ...
        # option2: select topk directly, without unique max
            ...
            k = min(log_action_prob.shape[-1], beam_size)
            log_action_prob, action_idx = mindspore.ops.TopK()(log_action_prob, k)
            next_r = next_r.view((batch_size, -1))
            next_r = next_r[mindspore.numpy.arange(next_r.shape[0])[:, None], action_idx].view(-1)
            next_te = next_te.view((batch_size, -1))
            next_te = next_te[mindspore.numpy.arange(next_te.shape[0])[:, None], action_idx].view(-1)
            log_option_prob = log_option_prob.view((batch_size, -1))
            log_option_prob = log_option_prob[mindspore.numpy.arange(log_option_prob.shape[0])[:, None], action_idx].view(-1)
            log_tailentity_prob = log_tailentity_prob.view((batch_size, -1))
            log_tailentity_prob = log_tailentity_prob[
                mindspore.numpy.arange(log_tailentity_prob.shape[0])[:, None], action_idx].view(-1)
            ...
            option_offset = option_offset.view((batch_size, -1))
            option_offset = option_offset[mindspore.numpy.arange(option_offset.shape[0])[:, None], action_idx].view(-1)
            action_offset = action_offset.view((batch_size, -1))
            action_offset = action_offset[mindspore.numpy.arange(action_offset.shape[0])[:, None], action_idx].view(-1)
        ...
        seen_nodes = mindspore.ops.Concat(1)((seen_nodes[action_offset], mindspore.ops.ExpandDims()(action[1], 1)))
    ...
    output_beam_size = int(action[0].shape[0] / batch_size)
