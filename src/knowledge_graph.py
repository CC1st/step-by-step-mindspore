...
class KnowledgeGraph(mindspore.nn.Cell):
    """
    ...
    def update_params(self, loss, step_size=0.5, first_order=False):
        ...
        #grads = mindspore.ops.GradOperation(loss, filter(lambda p: p.requires_grad, self.parameters()))
        return parameters_to_vector(filter(lambda p: p.requires_grad, self.get_parameters())) - parameters_to_vector(grads) * step_size
    ...
    def vectorize_relation_entity_space(self, data_dir):
            ...
            with open(input_path) as f:
                for line in f:
                    try:
                        ...
                        score = float(score)
                        #score = self.ms_cast(score, mindspore.float32)
        ...
        def vectorize_unique_r_space(unique_r_space_list, unique_r_space_size):
            ...
            unique_r_space = self.zeros((bucket_size, unique_r_space_size),mindspore.float32) + self.dummy_r
        ...
        def vectorize_relation_space(relation_space_list, relation_space_size):
            ...
            # r_space = self.zeros((bucket_size, relation_space_size), mindspore.int64)
            # r_mask = self.zeros((bucket_size, relation_space_size), mindspore.float32)
            r_space = np.zeros((bucket_size, relation_space_size), dtype=np.intc)
            r_mask = np.zeros((bucket_size, relation_space_size), dtype=np.float)
            for i, relation_space in enumerate(relation_space_list):
                relation_space = np.asarray(relation_space).astype(np.intc)
                for j, r in enumerate(relation_space):
                    r_space[i, j] = r
                    r_mask[i, j] = 1
            #return int_var_cuda(r_space), var_cuda(r_mask)
            r_space = mindspore.Tensor(r_space, mindspore.int32)
            r_mask = mindspore.Tensor(r_mask, mindspore.float32)
            return mindspore.Parameter(r_space, requires_grad=False),\
                   mindspore.Parameter(r_mask, requires_grad=False)
        ...
        def vectorize_tailentity_space(tailentity_space_list, tailentity_space_size):
            ...
            #e_space = torch.zeros(bucket_size, self.num_relations, tailentity_space_size) + self.dummy_e
            #te_mask = torch.zeros(bucket_size, self.num_relations, tailentity_space_size)
            te_space = np.zeros((bucket_size, self.num_relations, tailentity_space_size), dtype=np.intc)
            te_mask = np.zeros((bucket_size, self.num_relations, tailentity_space_size), dtype=np.float)
            for i, r_tailentity in enumerate(tailentity_space_list):
                for r in r_tailentity.keys():
                    for k, te in enumerate(r_tailentity[r]):
                        te_space[i, r, k] = te
                        te_mask[i, r, k] = 1
            te_space = mindspore.Tensor(te_space, mindspore.int32)
            te_mask = mindspore.Tensor(te_mask, mindspore.float32)
            return mindspore.Parameter(te_space, requires_grad=False),\
                   mindspore.Parameter(te_mask, requires_grad=False)
    ...
    def vectorize_action_space(self, data_dir):
        ...
        def vectorize_action_space(action_space_list, action_space_size):
            ...
            r_space = self.zeros((bucket_size, action_space_size),mindspore.float32) + self.dummy_r
            e_space = self.zeros((bucket_size, action_space_size),mindspore.float32) + self.dummy_e
            action_mask = self.zeros((bucket_size, action_space_size),mindspore.float32)
        ...
        def answers_to_var(d_l):
            ...
            for x in d_l:
                for y in d_l[x]:
                    ...
                    v = mindspore.ops.ExpandDims()(mindspore.Tensor(list(d_l[x][y]), mindspore.int32), 1)
                    d_v[x][y] = mindspore.Parameter(v, requires_grad=False)
    ...
    def get_all_entity_embeddings(self):
        ...
        return self.EDropout(self.entity_embeddings.embedding_table)
    ...
    def get_all_relation_embeddings(self):
        ...
        return self.RDropout(self.relation_embeddings.embedding_table)

    def get_relation_embeddings(self, r):
        return self.RDropout(self.relation_embeddings(r))

    def print_relation_embedding_weight(self):
        return self.relation_embeddings.embedding_table

    def get_all_entity_img_embeddings(self):
        #return self.EDropout(self.entity_img_embeddings.weight)
        return self.EDropout(self.entity_img_embeddings.embedding_table)
    ...
    def define_modules(self):
        if not self.args.relation_only:
            ...
            self.entity_embeddings = mindspore.nn.Embedding(self.num_entities, self.entity_dim)
            if self.args.model == 'complex':
                #self.entity_img_embeddings = nn.Embedding(self.num_entities, self.entity_dim)
                self.entity_img_embeddings = mindspore.nn.Embedding(self.num_entities, self.entity_dim)
            #self.EDropout = nn.Dropout(self.emb_dropout_rate)
            self.EDropout = mindspore.nn.Dropout(keep_prob=1-self.emb_dropout_rate)
        #self.relation_embeddings = nn.Embedding(self.num_relations, self.relation_dim)
        self.relation_embeddings = mindspore.nn.Embedding(self.num_relations, self.relation_dim)
        if self.args.model == 'complex':
            #self.relation_img_embeddings = nn.Embedding(self.num_relations, self.relation_dim)
            self.relation_img_embeddings = mindspore.nn.Embedding(self.num_relations, self.relation_dim)
        #self.RDropout = nn.Dropout(self.emb_dropout_rate)
        self.RDropout = mindspore.nn.Dropout(keep_prob=1-self.emb_dropout_rate)

    def initialize_modules(self):
        if not self.args.relation_only:
            #nn.init.xavier_normal_(self.entity_embeddings.weight)
            # self.entity_embeddings.embedding_table = initializer(Normal(sigma=0.01, mean=0.0),
            #                                                      self.entity_embeddings.embedding_table.shape,
            #                                                      mindspore.float32)
            self.entity_embeddings.embedding_table = initializer(XavierUniform(),
                                                                 self.entity_embeddings.embedding_table.shape,
                                                                 mindspore.float32)

        #nn.init.xavier_normal_(self.relation_embeddings.weight)
        # self.relation_embeddings.embedding_table = initializer(Normal(sigma=0.01, mean=0.0),
        #                                                        self.relation_embeddings.embedding_table.shape,
        #                                                        mindspore.float32)
        self.relation_embeddings.embedding_table = initializer(XavierUniform(),
                                                               self.relation_embeddings.embedding_table.shape,
                                                               mindspore.float32)
