...
mindspore.context.set_context(device_target='GPU', device_id=args.gpu, mode=context.PYNATIVE_MODE,
                              save_graphs=False, save_graphs_path="/home/luoxuewei/Project/Code3/ms_graphs_1")
...
def run_experiment(args):
    ...
    if args.process_data:
        ...
        #with torch.set_grad_enabled(args.train):
            initialize_model_directory(args)
            lf = construct_model(args)
            # ms指定设备后，数据和模型默认拷贝到指定的设备上，无需也不能再指定运行的设备
            # lf.cuda()
